...
class ConnectionManager {
	...
	constructor(serialIO, logger, config, urcHandler,broadcastHandler) { 
		this.logger = logger;
...
//		this.tcp
		...
		this.atProcedures.registerTcpMqttRecHandler("connMgr.HandleIncommingTCP", 
			...
			function(mqtt) { 
				...
				var mqttStr = mqtt.retMsg.msg.replace(re, "");
				mqttStr = mqttStr.replace(/Â/gi,'');
				var mqttObj = JSON.parse(mqttStr);
				console.log("General TCP Handler:", mqttObj); 

				if(mqttObj.type && mqttObj.type === 'tagupdate') {
					that.logger.system("Sending message  to Animation Window: " + JSON.stringify(mqttObj));
					that.broadcastHandler.sendData("sierrademo.suitcase",mqttObj);
				}
			}
		);
...
*/
	...
	async close() {
		await this.atProcedures.cleanup();
	}
